...
sub delivery_service_data_by_profile {
	...
	my $qry = 'select
		...
		deliveryservice.slice_blockbytes,
	...
	my $deliveryservice_slice_blockbytes;
	...
	$stmt->bind_columns(
		...
		\$deliveryservice_slice_blockbytes,
	...
	while ( my $row = $stmt->fetch() ) {
		push(
				@data, {
					...
					"slice_blockbytes" => $deliveryservice_slice_blockbytes,
...
sub profile_ds_data {
	...
	while ( scalar(@rs) !=0 ) {
		...
		my $slice_blockbytes            = $row->{'slice_blockbytes'};
		...
		$dsinfo->{dslist}->[$j]->{"slice_blockbytes"}            = $slice_blockbytes;
...
sub ds_data {
	...
	while ( my $dsinfo = $rs_dsinfo->next ) {
		...
		my $slice_blockbytes            = $dsinfo->slice_blockbytes;
		...
		$response_obj->{dslist}->[$j]->{"slice_blockbytes"}            = $slice_blockbytes;
...
sub remap_dot_config {
	...
	if ( $server_obj->type->name =~ m/^MID/ ) {
		...
		foreach my $ds ( @{ $data->{dslist} } ) {
			...
			if ( $ds->{range_request_handling} == RRH_CACHE_RANGE_REQUEST || $ds->{range_request_handling} == RRH_SLICE ) {
...
sub build_remap_line {
	...
	elsif ( $remap->{range_request_handling} == RRH_SLICE ) {
		$text .= " \@plugin=slice.so ";
		if ( defined( $remap->{slice_blockbytes} ) && $remap->{slice_blockbytes} ne "" ) {
			$text .= " \@pparam=blockbytes:" . $remap->{slice_blockbytes};
		}
		$text .= " \@plugin=cache_range_requests.so ";
	}
