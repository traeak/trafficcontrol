...
func (v *TODeliveryServiceV13) DeleteQuery() string {
...
func (ds *TODeliveryServiceV13) GetXMLID(tx *sqlx.Tx) (string, bool, error) {
	if ds.ID == nil {
...
func (ds *TODeliveryServiceV13) IsTenantAuthorized(user *auth.CurrentUser) (bool, error) {
	return isTenantAuthorized(user, ds.ReqInfo.Tx, &ds.DeliveryServiceNullableV13)
}
...
func getTenantID(tx *sql.Tx, ds *tc.DeliveryServiceNullableV13) (*int, error) {
...
func isTenantAuthorized(user *auth.CurrentUser, tx *sqlx.Tx, ds *tc.DeliveryServiceNullableV13) (bool, error) {
...
func (ds *TODeliveryServiceV13) Read() ([]interface{}, error, error, int) {
	...
	if len(errs) > 0 {
		...
		return nil, nil, errors.New("reading ds v13: " + util.JoinErrsStr(errs)), http.StatusInternalServerError
	}
...
func (ds *TODeliveryServiceV13) Delete() (error, error, int) {
	...
	if err != nil {
		return nil, errors.New("dsv13 delete: getting xmlid: " + err.Error()), http.StatusInternalServerError
	} else if !ok {
	...
	if _, err := ds.ReqInfo.Tx.Tx.Exec(`DELETE FROM regex WHERE id IN (SELECT regex FROM deliveryservice_regex WHERE deliveryservice=$1)`, *ds.ID); err != nil {
		return nil, errors.New("TODeliveryServiceV13.Delete deleting regexes for delivery service: " + err.Error()), http.StatusInternalServerError
	}

	if _, err := ds.ReqInfo.Tx.Tx.Exec(`DELETE FROM deliveryservice_regex WHERE deliveryservice=$1`, *ds.ID); err != nil {
		return nil, errors.New("TODeliveryServiceV13.Delete deleting delivery service regexes: " + err.Error()), http.StatusInternalServerError
	}
	...
	if _, err := ds.ReqInfo.Tx.Tx.Exec(`DELETE FROM parameter WHERE name = 'location' AND config_file = ANY($1)`, pq.Array(configFiles)); err != nil {
		return nil, errors.New("TODeliveryServiceV13.Delete deleting delivery service parameteres: " + err.Error()), http.StatusInternalServerError
	}
